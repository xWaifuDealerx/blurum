...
export default function Chat({ xmtp, profile, onMessage, onReact, toast }: any) {
  ...
  const statusPill = () => {
    const map: any = { live: ["live", "🟢 Live on XMTP · #general"], connecting: ["connecting", "Starting secure chat…"], needgroup: ["connecting", "Connected · join #general"], error: ["demo", "XMTP unavailable — see console"], idle: ["", "not started"] };
  ...
  return (
    <section className="view active">
      ...
      {status === "idle" || status === "connecting" ? (
        ...
        <div className="empty"><div className="box"><div className="big">🚪</div><h3>Join #general</h3><p>If your community already has a #general, ask a member to add your inbox. Founders can create it.</p><div className="idpill">your inbox: {short(inboxId || "")}</div><div style={{ marginTop: 16, display: "flex", gap: 9, justifyContent: "center", flexWrap: "wrap" }}><button className="btn primary" onClick={requestJoin}>🚪 Request to join</button><button className="btn ghost" onClick={createGeneral}>Create #general</button><button className="btn ghost" onClick={start}>Refresh</button></div></div></div>
      ) : status === "error" ? (
