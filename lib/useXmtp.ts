...
export function useXmtp(toast: (h: string, e?: string) => void) {
  ...
  const [status, setStatus] = useState<"idle" | "connecting" | "joining" | "needgroup" | "live" | "error">("idle");
  ...
  // Are we already a member of the ONE #general room?
  const enterIfMember = useCallback(async () => {
    const c = clientRef.current; if (!c) return false;
    await c.conversations.sync();
    const gid = GROUP_ENV || (typeof localStorage !== "undefined" ? localStorage.getItem("blurum:groupId") : "") || "";
    if (!gid) return false;
    let g: any = null; try { g = await c.conversations.getConversationById(gid); } catch {}
    if (g) { groupRef.current = g; localStorage.setItem("blurum:groupId", g.id); setStatus("live"); await loadMessages(); streamLoop(); return true; }
    return false;
  }, [loadMessages, streamLoop]);

  const ensureGroup = useCallback(async () => {
    if (await enterIfMember()) return;
    // There is exactly ONE room (#general). If it's configured and we're not in yet,
    // auto-join it via the greeter ("doorman") — users never create their own room.
    if (GROUP_ENV && process.env.NEXT_PUBLIC_GREETER_ADDRESS) { setStatus("joining"); requestJoin(); return; }
    setStatus("needgroup"); // first run only: the founder creates the single #general once
  }, [enterIfMember]);
  ...
  const requestJoin = useCallback(async () => {
    ...
    if (!greeter) { setStatus("needgroup"); return; }
    try {
      setStatus("joining");
      let dm: any = null;
      ...
      for (let i = 0; i < 15; i++) { await new Promise((r) => setTimeout(r, 2500)); if (await enterIfMember()) { toast("You’re in #general 🎉", "🎉"); return; } }
      setStatus("needgroup"); toast("Couldn’t reach the #general doorman — is the greeter running?", "⏳");
    } catch (e: any) { setStatus("needgroup"); toast("Join failed: " + (e?.message || e), "⚠️"); }
  }, [enterIfMember, toast]);
